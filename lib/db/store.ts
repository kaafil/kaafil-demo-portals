 ...
 *
 *   seedStore(fixture)  destroys crm.sqlite and rebuilds it. `pnpm seed`.
 *   openStore()         opens what is already there, read-only. Route handlers.
 *
 * `openStore()` refuses rather than seeds if the file is missing, because a
 * route handler that silently seeds would turn "I forgot to run pnpm seed"
 * into a slow first request instead of a clear error.
 *
 * Note the asymmetry, inherited deliberately: THIS store is cheap to destroy
 * and is rebuilt from fixtures whenever you like; the Kaafil tenant is never
 * touched automatically. Pushing to the engine is `pnpm seed:kaafil`,
 * explicit and separate, because that call is rate limited and — on a live
 * key — has no undo.
 *
...
/**
 * The store sits at the repo root, next to `.env`.
 *
 * Resolved from `process.cwd()` rather than `import.meta.url` on purpose: this
 * module is imported both by `tsx scripts/seed.ts` and by a Next route handler
 * that has been through a server bundle, and only one of those two reliably
 * knows its own file URL. Both run from the repo root.
 */
...
// below use all four.
import type { StaffRecord, TourDetail, TourSummary, TravellerRecord } from '@/config/contract';
...
/**
 * Destroys `crm.sqlite` and rebuilds it from `fixture`. This is `pnpm seed`,
 * and it is the ONLY thing in the repo that writes to the CRM's database.
 *
 ...
 * first request; this turns it into a sentence telling you what to type.
 */
export function openStore(): CrmStore {
  if (!existsSync(DB_PATH)) {
    throw new Error(
      `crm.sqlite is not there (looked in ${DB_PATH}). Run \`pnpm seed\` to build it. ` +
        '`pnpm dev` does that for you; `pnpm dev:only` deliberately does not.',
    );
  }
  ...
  // Counted off the open database rather than off a fixture. `openStore()` is
  // handed no fixture at all now, and a count read from the rows is the one
  // that can never disagree with what a query will return.
  const countRow = (table: string): number =>
    (db.prepare(`SELECT COUNT(*) AS n FROM ${table}`).get() as { n: number }).n;

  const counts: StoreCounts = {
    staff: countRow('staff'),
    tours: countRow('tours'),
    bookings: countRow('bookings'),
    payments: countRow('payments'),
    travellers: countRow('travellers'),
  };
